...
# Model 1(1+) stabilized host-parasite-hyperparasite cycle
...
pdf('Fig3.pdf',paper="a4r",width=0,height=0)
...
for (k in 1:m) {
  ...
  P  <- r$P[k]  + dt * ((c * r$R2[k] * r$P[k]  - delta_3 * r$F2[k] *r$P[k])  * Pi_uppercase + alpha - d_P  * r$P[k])
  F1 <- r$F1[k] + dt * ((d * r$F2[k] * r$P[k]  + beta    * r$R2[k] *r$P[k])  * Pi_uppercase         - d_F1 * r$F1[k])
  F2 <- r$F2[k] + dt * ( e * r$R1[k] * r$F1[k]                               * Pi_uppercase         - d_F2 * r$F2[k])
  t  <- r$t[k] + dt
...
legend("topright",
       ...
       lty = c(1, 1, 1, 1, 1, 2),
       col = c("green","cyan","blue","red","pink","grey"),
       box.lwd = 0)
